...
variable "Location_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Location_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-location"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Location_group" {
  ...
  name = "${var.environment}_YCU_Location"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Location_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Location"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Location_configuration" {
  name                  = "${var.environment}_Location"
  image_id              = "${coalesce(lookup(var.Location_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Location_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Location_user_data.rendered}"
}
