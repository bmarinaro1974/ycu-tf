variable "Wellness-Services_ami_ids" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
variable "Wellness-Services_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Wellness-Services_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-wellness-services"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Wellness-Services_group" {
  ...
  name = "${var.environment}_YCU_Wellness-Services"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Wellness-Services_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Wellness-Services"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Wellness-Services_configuration" {
  name                  = "${var.environment}_Wellness-Services"
  image_id              = "${coalesce(lookup(var.Wellness-Services_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Wellness-Services_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Wellness-Services_user_data.rendered}"
}
