...
variable "Profile_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Profile_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-profile"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Profile_group" {
  ...
  name = "${var.environment}_YCU_Profile"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Profile_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Profile"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Profile_configuration" {
  name                  = "${var.environment}_Profile"
  image_id              = "${coalesce(lookup(var.Profile_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Profile_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Profile_user_data.rendered}"
}
