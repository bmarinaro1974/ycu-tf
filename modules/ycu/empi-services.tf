variable "Empi-Services_ami_ids" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
variable "Empi-Services_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Empi-Services_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-empi-services"
    domain = "${var.domain}"
...
resource "aws_launch_configuration" "Empi-Services_configuration" {
  name                  = "${var.environment}_Empi-Services"
  image_id              = "${coalesce(lookup(var.Empi-Services_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Empi-Services_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Empi-Services_user_data.rendered}"
}

resource "aws_autoscaling_group" "Empi-Services_group" {
  ...
  name = "${var.environment}_YCH_Empi-Services"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Empi-Services_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCH_Empi-Services"
