variable "Legacy-Empi-Service_ami_ids" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
variable "Legacy-Empi-Service_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Legacy-Empi-Service_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-legacy-empi-service"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Legacy-Empi-Service_group" {
  ...
  name = "${var.environment}_YCU_Legacy-Empi-Service"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Legacy-Empi-Service_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Legacy-Empi-Service"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Legacy-Empi-Service_configuration" {
  name                  = "${var.environment}_Legacy-Empi-Service"
  image_id              = "${coalesce(lookup(var.Legacy-Empi-Service_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Legacy-Empi-Service_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Legacy-Empi-Service_user_data.rendered}"
}
