...
variable "YCI-50_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "YCI-50_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-yci-50"
    domain = "${var.domain}"
...
resource "aws_launch_configuration" "YCI-50_configuration" {
  name                  = "${var.environment}_YCI-50"
  image_id              = "${coalesce(lookup(var.YCI-50_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.YCI-50_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.YCI-50_user_data.rendered}"
}


resource "aws_autoscaling_group" "YCI-50_group" {
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.YCI-50_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCI-50"
