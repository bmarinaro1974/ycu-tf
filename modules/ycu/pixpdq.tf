...
variable "PixPDQ_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "PixPDQ_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-pixpdq"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "PixPDQ_group" {
  ...
  name = "${var.environment}_YCU_PixPDQ"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.PixPDQ_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_PixPDQ"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "PixPDQ_configuration" {
  name                  = "${var.environment}_PixPDQ"
  image_id              = "${coalesce(lookup(var.PixPDQ_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.PixPDQ_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.PixPDQ_user_data.rendered}"
}
