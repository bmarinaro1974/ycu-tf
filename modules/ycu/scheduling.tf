...
variable "Scheduling_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Scheduling_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-scheduling"
    domain = "${var.domain}"
...
resource "aws_launch_configuration" "Scheduling_configuration" {
  name                  = "${var.environment}_Scheduling"
  image_id              = "${coalesce(lookup(var.Scheduling_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Scheduling_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Scheduling_user_data.rendered}"
}

resource "aws_autoscaling_group" "Scheduling_group" {
  ...
  name = "${var.environment}_YCU_Scheduling"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Scheduling_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Scheduling"
