...
variable "Security_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Security_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-security"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Security_group" {
  ...
  name = "${var.environment}_YCU_Security"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Security_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Security"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Security_configuration" {
  name                  = "${var.environment}_Security"
  image_id              = "${coalesce(lookup(var.Security_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Security_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Security_user_data.rendered}"
}
