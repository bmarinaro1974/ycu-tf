...
variable "Facility-50_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Facility-50_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-facility-50"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Facility-50_group" {
  ...
  name = "${var.environment}_YCE_Facility-50"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Facility-50_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCE_Facility-50"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Facility-50_configuration" {
  name                  = "${var.environment}_Facility-50"
  image_id              = "${coalesce(lookup(var.Facility-50_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Facility-50_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Facility-50_user_data.rendered}"
}
