...
variable "Notification_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Notification_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-notification"
    domain = "${var.domain}"
...
resource "aws_launch_configuration" "Notification_configuration" {
  name                  = "${var.environment}_Notification"
  image_id              = "${coalesce(lookup(var.Notification_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Notification_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Notification_user_data.rendered}"
}


resource "aws_autoscaling_group" "Notification_group" {
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Notification_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_Notification"
