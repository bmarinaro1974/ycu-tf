...
variable "Event-Sync_instance_types" {
  description = ""
  type        = "map"
  default     = {
    dev1=""
...
resource "template_file" "Event-Sync_user_data" {
  template = "${file("${path.module}/user-data.sh")}"

  vars {
    chef_role = "microservice-event-sync"
    domain = "${var.domain}"
...
resource "aws_autoscaling_group" "Event-Sync_group" {
  ...
  name = "${var.environment}_YCU_Event-Sync"
  ...
  force_delete = true
  launch_configuration = "${aws_launch_configuration.Event-Sync_configuration.id}"

  tag {
    key = "Name"
    value = "${var.environment}_YCU_Event-Sync"
    propagate_at_launch = true
  }
}

resource "aws_launch_configuration" "Event-Sync_configuration" {
  name                  = "${var.environment}_Event-Sync"
  image_id              = "${coalesce(lookup(var.Event-Sync_ami_ids, var.environment), lookup(var.default_ami_ids, var.environment))}"
  instance_type         = "${coalesce(lookup(var.Event-Sync_instance_types, var.environment), lookup(var.default_instance_types, var.environment))}"
  ...
  user_data             = "${template_file.Event-Sync_user_data.rendered}"
}
